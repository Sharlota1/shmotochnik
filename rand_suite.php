<?php
/**
*@file Генерация рандомного костюма
*
*@autor Быкова Алёна
*/
require 'bd.php';
...
if(IsChecked('cloth_type','casual')){
   ...
   $r=select('Повседневный стиль', 'Костюм');
   if(!empty($r)){
     display($r);
  }
 ...
 if(IsChecked('cloth_type','oficial')){
  $r=select('Официальный/вечерний стиль', 'Костюм');
   if(!empty($r)){
     display($r);
  }
}
...
if(IsChecked('cloth_type','sport')){
  $r=select('Спортивный стиль', 'Костюм');
   if(!empty($r)){;
     display($r);
  }
