<?php
/**
*@file Генерация рандомной верхней части одежды
*
*@autor Быкова Алёна
*/
require 'bd.php';
...
if(IsChecked('cloth_type','casual')){
   ...
   $r=select('Повседневный стиль', 'Верх');
   if(!empty($r)){
   display($r);
  }
 ...
 if(IsChecked('cloth_type','oficial')){
  $r=select('Официальный/вечерний стиль', 'Верх');
   if(!empty($r)){
     display($r);
  }
}
...
if(IsChecked('cloth_type','sport')){
  $r=select('Спортивный стиль', 'Верх');
   if(!empty($r)){;
     display($r);
  }
